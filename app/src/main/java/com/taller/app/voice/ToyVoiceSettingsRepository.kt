...
class ToyVoiceSettingsRepository(private val context: Context) {
    ...
    suspend fun save(settings: ToyVoiceSettings) {
        context.toyVoiceDataStore.edit { prefs ->
            ...
            if (settings.openAiVoiceName != null) {
                prefs[Keys.OPENAI_VOICE_NAME] = settings.openAiVoiceName
            } else {
                prefs.remove(Keys.OPENAI_VOICE_NAME)
            }
            if (settings.openAiInstructions != null) {
